...
### fit marginal distributions by Gaussian, here ATT_Ret is a vector
est.ATT = as.numeric(fitdistr(ATT_Ret,"normal")$estimate) # mean and sd
est.VZ  = as.numeric(fitdistr(VZ_Ret,"normal")$estimate)

# we transform data by fitting marginal distributions, creating new data ranging (0,1)
...
pairs(cbind(ATT_Ret,VZ_Ret,S_Ret),pch=20)
    ...
    # these are normal transformed returns plot. 
    ...
    plot(data1[,1],data1[,2], pch=20,
        ...
        ####### fit marginal distributions by T-distribution, we got data2 and data3
